(async function () {
  const main = () => {
    ...
    const copyBranchName = async () => {
      const task = "task-" + location.href.match(/task-view-(\d+)/)![1];
      const date = timeFormat(new Date());
      const branch = `yeli/${date}/${task}/`;
      await navigator.clipboard.writeText(branch);
      toast("分支名已复制到粘贴板: " + branch);

      let maps: string[][] = await chrome.storage.sync.get("maps").then((res) => res.maps || []);
      maps = [...maps, [task, document.title]].filter((e, i, arr) => arr.findIndex((a) => a[0] === e[0]) === i).splice(10, maps.length - 10);
  ...
  const isTargeUrl = () => location.href.includes("task-view-");
